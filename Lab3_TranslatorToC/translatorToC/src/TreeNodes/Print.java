...
public class Print extends NodeClass implements Node {
    ...
    public String toString(int padding) {
        ...
        } else if(expr instanceof MatExpression) {
            Var tempPrintVar = new Var("__temp_print_var" + expr.hashCode(), Types.Bool);
            NewVar tempNewVar = new NewVar(ConstOrVar.LET, tempPrintVar, ((MatExpression)expr).getType(), (MatExpression)expr);
            String result = tempNewVar.toString(padding);
            result += getPadding(padding) + "printf(\"%" + ((MatExpression) expr).getType().getCCode() + "\\n\", " + tempPrintVar.getName() + ");" + "\n";
            return result;
        }
        throw new Error();
    }
