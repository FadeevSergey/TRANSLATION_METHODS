...
newVar returns [NewVar expr]:
    ...
    | mut=constOrVar WHITE_SPACE var=variable[Types.Unknown] WHITE_SPACE* EQ_TOKEN WHITE_SPACE* right=eqRight[$var.var]
      {
          if($right.right instanceof MatExpression) {
...
mathExpression returns [MatExpression expr]:
      RLB_TOKEN WHITE_SPACE* mtExpr=mathExpression WHITE_SPACE* RRB_TOKEN
      {
          $expr=new MatExpression($mtExpr.text, $mtExpr.expr.getType());
      }
    | left=mathExpression WHITE_SPACE* op=mathOperators WHITE_SPACE* right=mathExpression
      {
          if($left.expr.getType() == Types.Double || $right.expr.getType() == Types.Double) {
              $expr=new MatExpression($left.text + " " + $op.text + " " + $right.text, Types.Double);
          } else {
              $expr=new MatExpression($left.text + " " + $op.text + " " + $right.text, Types.Int);
          }

      }
    | var=variable[Types.Unknown]
      {
          $expr=new MatExpression($var.var.toString(), Types.Unknown);
      }
    | INTEGER
      {
          $expr=new MatExpression($INTEGER.text, Types.Int);
      }
    | DECIMAL
      {
          $expr=new MatExpression($DECIMAL.text, Types.Double);
      }
